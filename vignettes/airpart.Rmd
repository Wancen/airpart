...
though). If users want to estimate homogeneous cell type allelic imbalance, 
can set `sd = 0` and look at below summary step to find interested gene clusters
whose weighted mean deviate from 0.5.
...
Answer: Clustering genes have several good things: 
* More power on cell type partition
* Narrow down the confidence interval which may be more accurate on giving allelic imbalance inference
* speed concern
...
Answer: This step is actually a complement of QC on genes step in case still some genes not filtered out. 
And I add homogeneous selection choice at the above QC on genes last sentences. 
...
Answer: If we look at diff groups as a mixture of beta distributions. Then we modelled BB within each group so that they have the same overdispersion parameter. But I think it's ok that diff cell types within a group have a little bit difference. The way fussed lasso works is also fused cell types together when the difference below some threshold.
...
complex simulation, with 8 cell types, in 3 true groups disordered by allelic ratio.

TODO: need to work on this text

Answer: I change the p.vec value so that to show we can either 
display result by group order or cell type order. 
I am thinking if we will have tens of cell types in the future, use graph to show the network is better.
...
To do: I want to also split heatmap in this case, but I don't know how to detect change point on factors without importing another package.
...
Answer: When users only want to find DAI for some specific well-known genes? Or they don't trust the gene cluster result which returns too narrow confidence interval?
...
fl <- pbsapply(1:dim(sce_gene)[1], function(i) {
  sce_genesub <- airpart::fusedLasso(sce_gene,model="binomial",
