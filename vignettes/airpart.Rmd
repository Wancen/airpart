...
above, it give fused lasso estimate or weighted mean as prior Cauchy mean to estimate the allelic ratio per gene and per cell type (and confidence intervals) via beta-binomial loglikelihood, using functionality from the *apeglm* package.
...
```
Allelic ratio estimate(`ar`) as well as `s` value and confidence interval (`lower` and `upper`) are stored in `rowData`.
```{r results="asis"}
