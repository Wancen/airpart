...
```{r, warning=FALSE, results="asis"}
# To do: default is using percentile method, cannot work with R=20, otherwise change to normal
# bootstrap method
sce_sub <- allelicRatio(sce_sub, method="bootstrap", R=100, ncpus=4, parallel="multicore")
knitr::kable(metadata(sce_sub)$estimator, row.names=FALSE)
```
...
sce_sub <- consensusPart(sce_sub)
thrs <- 10^seq(from=-2,to=-0.4,by=0.1)
sce_sub <- wilcoxExt(sce,genecluster=1,threshold=thrs) # faster alternative
sce_sub <- allelicRatio(sce_sub)
