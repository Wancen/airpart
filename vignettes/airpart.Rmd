...
# Real Data example
Vignette on Larsson 2019 data can be found in [here](https://htmlpreview.github.io/?https://github.com/Wancen/airpartpaper/blob/main/R/Larsson2019.html) which has Single-cell allelic RNA-seq with 4 cell states.

# Overview of Toy example data
...
Note that the estimates and credible intervals are not equal for
...
```
Allelic ratio estimate(`ar`) as well as `s` value and credible interval (`lower` and `upper`) are stored in `rowData`.
```{r results="asis"}
genepoi<-paste0("gene",seq_len(5))
knitr::kable(rowData(sce_sub)[genepoi,c(grep('ar', colnames(rowData(sce_sub)), value=TRUE))])
```
...
## but can set ctpoi=c(1,3,7) to limit cell types be plotted when there is too many cell types.
makeForest(sce_sub) 
```
