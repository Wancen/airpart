...
label<-tibble(type=cell_meta_unique,par=factor(class))
...
annotation_col = celltypeQC2,show_colnames = F,show_rownames = F)
...
label<-tibble(type=cell_meta_unique,par_fused=class,par_wilcoxon=factor(fit2))
...
coef_bb <- coef_bb + c(0,rep(coef_bb[1],length(coef_bb)-1))
coef_wilcoxon<-1/(1+exp(-coef_bb))
estimator<-tibble(par_wilcoxon=seq_len(length(coef_wilcoxon)),estimator=coef_wilcoxon)
estimator2<-merge(label, estimator, by="par_wilcoxon", all = T)
print(knitr::kable(estimator2))
```
...
```
```{r}
# empirical bayes using methods of moment
mu <- summary$mean
vr <- summary$var
...
alpha
beta
size<-dat %>% group_by(x) %>% summarise(n=sum(cts)) %>% data.frame()
binom.bayes(x = estimator$estimator*size$n, n = size$n, tol = 1e-8,prior.shape1 = alpha,prior.shape2 = beta)
```
