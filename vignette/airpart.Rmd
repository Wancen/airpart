...
sce <- makeSimulatedData(mu1=1,mu2=10,nct=4,n=20,ngenecl=25,theta=20,ncl=3,p.vec=p.vec)
```
...
sce_sub <- fusedLasso(formula=f,model="binomial",sce,genecluster=1,ncores=2,niter=2)
```
...
f <- ratio ~ p(x, pen = "gflasso") # formula for the GFL
sce_sub <- fusedLasso(formula=f,model="binomial",sce,genecluster=1,ncores=4,niter=2) 
sce_sub <- consensusPart(sce_sub)
thrs <- 10^seq(from=-2,to=-0.4,by=0.1)
