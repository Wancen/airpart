...
sce <- makeSimulatedData(mu1=1,mu2=10,nct=4,n=20,
...
```{r}
# TODO I'd recommend to use the 'keep' style of filtering
# instead of the 'remove' logic.
# you already have 'keep' logic in the feature QC below.
# Defining 'keep' is widely used paradigm: e.g. DESeq2, edgeR/limma, apeglm, swish, etc.
remove_cell <- (
...
```{r, warning=FALSE, results="asis"}
# TODO let's scale this back so vignette compiles faster
# less R?
sce_sub <- allelicRatio(sce_sub, method="bootstrap", R=200, ncores=4)
```
...
sce <- makeSimulatedData(mu1=1,mu2=10,nct=nct,n=30,ngenecl=50,theta=20,ncl=3,p.vec=p.vec)
sce <- preprocess(sce)
