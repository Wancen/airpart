...
t <- system.time({
  cl_gfl <- fusedlasso(formula=f,model="binomial",data=dat,ncores=4,niter = 1)
  })[[3]]
...
is smaller than 0.1 and therefore there will be uncertainty.
...
estimator <- sapply(1:max(cl_gfl), function(m) {
  bb <- vglm(cbind(ratio*cts, cts-ratio*cts) ~ 1, betabinomial,
             data = dat2[which(dat2$par_wilcoxon==m),], trace = FALSE)
