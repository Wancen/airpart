...
t<-system.time(cl<-fusedlasso(formula=f,model="binomial",data=dat,ncores=4,niter = 5))[[3]]
print(knitr::kable(cl %>% data.frame()))
```
Here `ncores` is the number of CPU used for parallelization.You can specify `niter=1` which is the default when the CTS weighted allelic ratio difference large than 0.1 and therefore there won't have uncertainty.
