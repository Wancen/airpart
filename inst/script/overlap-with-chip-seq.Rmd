...
get_overlap<-function(gene_prom,feat,mef,mESC){
  ...
  enrich<-merge(mESC_ov2,mef_ov2,by="sym",all=TRUE)
  return(enrich)
}
```
...
for (i in 1:28) {
  ...
  summary<-dat %>% group_by(x) %>% summarise(weighted.mean=weighted.mean(ratio,cts,na.rm = TRUE),mean=mean(ratio,na.rm = TRUE),var=var(ratio,na.rm = TRUE)) %>% data.frame()
