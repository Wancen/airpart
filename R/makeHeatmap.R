...
#' 
#' @export
makeRatioHeatmap <- function(se, show_row_names = FALSE, ...) {
  m <- assays(se)[["ratio_pseudo"]]
  if (is.null(se$part)) {
    ha <- HeatmapAnnotation(`cell type` = se$x, border = FALSE,
                            col = list(`cell type` = structure(brewer.pal(nlevels(se$x), "Set3"),
                                                               names = levels(se$x))))
