...
#'    exceed \code{mad_spikegenes} median absolute deviations from the median spike genes expression percentage for the dataset}
#'   }
#'
#' @examples
...
cellQC <- function(sce, spike, threshold=0,
  ...
  if(missing(spike)){
    ...
    spikePercent<-colSums(counts(sce)[spike_gene,],na.rm = T)*100/colSums(counts(sce)[-spike_gene,],na.rm = T)
  ...
  coldata<-cbind(colData(sce),sum,detected,spikePercent,filter_sum,filter_detected,filter_spike)
  coldata
}
...
#' percentage of cells expressed within each cell type. Default is 25%
...
featureQC <- function(sce, spike, threshold=0.25, lowsd=0.04,pc=2) {
  ...
  check<-pbsapply(levels(sce$x),function(c){
    poi<-which(sce$x==c)
    ct_threshold<-nexprs(counts(sce), byrow=TRUE,  detection_limit=1,subset_col=poi)>= length(poi)*threshold
    weighted_mean<-rowSums(assays(sce[,poi])[["ratio_pseudo"]]*(counts(sce[,poi])+2*pc)/rowSums(counts(sce[,poi])+2*pc))
  ...
  filter_spike<-rep(T,nrow(sce))
  if(!missing(spike)){
    filter_spike[grep(paste0("^",spike), row.names(sce))]<-F
  }
