...
allelicRatio<-function(se,level=0.95,method=c("Normal","bootstrap"),
  ...
  if(method=="bootstrap"){
    ...
    coef<-boot$t0
    confint<-t(confint) %>% as.data.frame() %>% setNames(paste(c((1-level)*50,100 - (1-level)*50),"%"))
  }else{
...
betaBinom <- function(data,ci=TRUE) {
  ...
  res<-sapply (1:nlevels(data$part), function(m){
    suppressWarnings(bb<-VGAM::vglm(cbind(ratio*cts, cts-ratio*cts) ~1, VGAM::betabinomial, data = data[which(data$part==m),],
                                    trace = F))
